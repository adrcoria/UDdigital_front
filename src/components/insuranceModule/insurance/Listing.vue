...
import {
  headers,
  actions,
} from "@/components/insuranceModule/insurance/utils";

import Table from "@/app/common/components/Table.vue";
import { InsuranceType } from "@/components/insuranceModule/insurance/types";
import CreateEditDialogObj from "@/components/insuranceModule/insurance/CreateEditDialog.vue";
import RemoveItemConfirmationDialog from "@/app/common/components/RemoveItemConfirmationDialog.vue";
import { insuranceService } from "@/app/http/httpServiceProvider";
...
const selectedDetail = ref<InsuranceType | null>(null);

const confirmationDialog = ref(false);
const confirmationObj = ref<InsuranceType | null>(null);

const isAllChecked = ref(false);

const filtered = ref<InsuranceType[]>([]);
const finalData = ref<InsuranceType[]>(filtered.value);

const page = ref(1);
const noOfItems = computed(() => {
  return finalData.value.length;
});
const tableData = ref<InsuranceType[]>([]);
...
const get = async () => {
  try {
    loading.value = true;
    const response = await insuranceService.get(); // Llamada al servicio
    ...
    // console.error(error);
    // const msg = getBackendErrorMessage(error, "Error al obtener aseguradoras");
    // showErrorAlert(msg);
  } finally {
    loading.value = false;
  }
...
const onSelect = (option: string, data: InsuranceType) => {
  if (option === "edit") {
...
const onUpdate = async (updatedVal: InsuranceType) => {
  try {

    loading.value = true;
    const response = await insuranceService.update(updatedVal, updatedVal.uuid);
    ...
    if ([200, 201].includes(response.data.statusCode)) {
      ...
      showErrorAlert('(' + response.data.data + ')', 111);
    }
  } catch (error) {
    const msg = getBackendErrorMessage(error, "Error al actualizar aseguradora");
    showErrorAlert(msg, 112);
  }
  finally {
    loading.value = false;
...
const onCreate = async (newVal: InsuranceType) => {
  try {

    loading.value = true;
    const response = await insuranceService.create(newVal);
    if ([200, 201].includes(response.data.statusCode)) {
      ...
      showErrorAlert('(' + response.data.data + ')', 113);
    }
  } catch (error) {
    const msg = getBackendErrorMessage(error, "Error al crear aseguradora");
    showErrorAlert(msg, 114);
  }
  finally {
    loading.value = false;
    createEditDialog.value = false;
...
const onConfirmDelete = async () => {
  try {
    ...
    loading.value = true;
    const response = await insuranceService.delete(confirmationObj?.value?.uuid);
    if ([200, 201].includes(response.data.statusCode)) {
      ...
      showErrorAlert('(' + response.data.data + ')', 115);
    }
  } catch (error) {
    const msg = getBackendErrorMessage(error, "Error al eliminar aseguradora");
    showErrorAlert(msg, 116);
  }
  finally {
    loading.value = false;
...
const updateTableData = (newVal: InsuranceType[]) => {
  loading.value = true;
  const { itemsPerPage, page } = config.value;
  ...
  <v-card>
    <v-card-title class="text-subtitle-1 font-weight-bold d-flex justify-space-between align-center">
      ...
      <v-btn color="primary" elevation="0" class="mt-2" @click="onAddUserClick()">
        <i class="ph-plus-circle mx-1" /> Agregar aseguradora
      </v-btn>
    </v-card-title>
    <v-card-text>
