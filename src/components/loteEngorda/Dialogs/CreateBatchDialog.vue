...
const save = async () => {
  ...
  try {
    ...
    // CONTRATO API CORREGIDO: idBatchType
    const payload = {
      name: form.value.name.toUpperCase(),
      idBatchType: props.batchTypeId,
      idBovines: [] // Enviamos array vacío por defecto al crear
    };

    await batchService.createBatch(payload);
    
    showSuccessAlert("Lote de Engorda creado correctamente");
    emit("refresh");
  ...
  <v-dialog v-model="props.modelValue" max-width="450px" persistent>
    <v-card class="rounded-lg">
      <v-card-title class="pa-4 bg-primary text-white d-flex align-center">
        <v-icon icon="ph-layout" class="mr-3" />
        <span class="text-h6 font-weight-bold">Nuevo Lote de Engorda</span>
      </v-card-title>
      ...
      <v-card-actions class="pa-4">
        ...
        <v-btn color="primary" variant="flat" :loading="loading" @click="save">Guardar Lote</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>