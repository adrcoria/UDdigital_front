...
const isEditing = ref(false); // Flag para controlar la carga de datos
...
/* ------------------ Limpieza Reactiva de Variables ------------------ */
watch(() => form.value.bovineOriginId, (newId) => {
    if (isEditing.value) return; // Evita limpiar durante la carga de datos
...
watch(() => props.modelValue, async (val) => {
    ...
    if (val) {
        ...
        try {
            // 1. Cargamos catálogos primero para tener la lista de nombres de razas
            await loadData();
            ...
            if (props.item?.id) {
                ...
                // 3. Llenamos el formulario con IDs para que los selectores hagan match
                form.value = {
                    ...form.value,
                    ...serverItem,
                ...
                // 4. CORRECCIÓN VISUAL DE RAZAS:
                // Mapeamos para que el v-model tenga el ID y Vuetify muestre el Nombre del catálogo
                if (serverItem.raceAssignments?.length) {
                    selectedRaces.value = serverItem.raceAssignments.map((r: any) => ({
                        ...
                        percentage: parseInt(r.percentage), // Quitamos los decimales .00 para la vista
                        order: r.order
                    }));
                } else {
                    selectedRaces.value = [{ raceId: null, percentage: 100, order: 1 }];
...
const save = async () => {
    ...
    try {
        loading.value = true;
        const { gdpTotal, ...cleanPayload } = form.value;
        const finalPayload = {
            ...cleanPayload,
        ...
        showErrorAlert(error.response?.data?.message || "Error al procesar la solicitud");
    } finally {
        loading.value = false;
    }
};
            ...
            <v-card-text class="pa-6">
                <v-form ref="formRef">
                    <v-row dense>
                        ...
                        <v-col cols="12" md="3"><v-text-field label="GDP" v-model="form.gdpTotal" variant="outlined"
                        ...
                        <template v-if="form.bovineStatus === 'MUERTO'">
                            ...
                            <v-col cols="12" md="6"><v-text-field label="Comentarios *" v-model="form.deathComments"
                                    :rules="[rules.required]" variant="outlined" /></v-col>
                        </template>
