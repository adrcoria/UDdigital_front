...
const getBatches = async () => {
  try {
    ...
    const all: any[] = payload?.data || [];
    // Filtro client-side por si el backend no filtra por tipo
    batches.value = all.filter(
      (b) => !b.batchType?.id || b.batchType.id === props.filters.batchTypeId
    );
    config.value = {
      ...config.value,
  ...
  <v-card border elevation="0" class="rounded-lg">
    <v-card-title class="pa-4 bg-white d-flex align-center">
      <div class="d-flex align-center">
        <v-icon color="blue-darken-2" class="mr-2" size="28">ph-drop</v-icon>
        <span class="text-h6 font-weight-bold text-uppercase">Lotes de Producción de Leche</span>
      </div>
      <v-spacer />
      <v-btn color="blue-darken-2" prepend-icon="ph-plus-bold" @click="onCreate" height="44" class="px-6 font-weight-bold">
        Nuevo Lote
      </v-btn>
    </v-card-title>
      ...
      <Table v-model="page" :config="config" :headerItems="headers" :loading="loading" is-pagination>
        <template #body>
          <tr v-for="item in tableData" :key="item.id" class="batch-row">
            <td class="font-weight-bold text-blue-darken-2 py-4">{{ item.name }}</td>

            <td class="text-center">
              <v-chip color="blue-darken-2" variant="flat" size="small" class="font-weight-black">
          ...
          <tr v-if="!loading && tableData.length === 0">
            <td :colspan="headers.length" class="text-center py-10 text-grey-darken-1">
              <v-icon size="48" color="grey-lighten-2" class="mb-2">ph-drop</v-icon>
